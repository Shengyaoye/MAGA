...
class AFMLayer(nn.Module):
    def __init__(self, embedding_size, attention_factor=4, l2_reg=0.0, drop_rate=0.1):
...
class AFM(nn.Module):
    def __init__(self, feat_size, embedding_size, dnn_feature_columns,
        ...
        if self.use_attention:
            self.fm = MultiHeadAFMLayer(embedding_size, num_heads=4, attention_factor=16, drop_rate=0.2) 
        else:
            self.fm = FM()
        #self.fm返回的是二阶交叉部分
...
if __name__ == '__main__':
    ...
    # initial_head0_layer0_weight = model.fm.attention_weights[0][0].weight.data.clone()
    for epoch in range(epoches):
        ...
        # # 检查参数是否变化
        # print("参数是否更新:", 
        #     not torch.allclose(initial_head0_layer0_weight, model.fm.attention_weights[0][0].weight.data)
        # )
